# Copied and slightly modified from https://github.com/FangShancheng/ABINet/blob/main/transforms.py
...
class CVRandomRotation(object):
    ...
    def __call__(self, img):
        ...
        return cv2.warpAffine(img, M, (dst_w, dst_h), flags=flags, borderMode=cv2.BORDER_CONSTANT)
...
class CVRandomPerspective(object):
    ...
    def __call__(self, img):
        ...
        flags = get_interpolation() 
        img = cv2.warpPerspective(img, M, (max_x, max_y), flags=flags, borderMode=cv2.BORDER_CONSTANT)
        img = img[min_y:, min_x:]
        return img
...
class CVGeometry(object):
    def __init__(self, degrees=15, translate=(0.3, 0.3), scale=(0.5, 2.), 
                       shear=(45, 15), distortion=0.5, antiphase=0.05, p=0.5):
        self.p = p
    ...
    def __call__(self, img):
        if random.random() < self.p:
            img = np.array(img)
            img = Image.fromarray(self.transforms(img))
        if random.random() < self.antiphase:
            img = img.rotate(180)
        return img
